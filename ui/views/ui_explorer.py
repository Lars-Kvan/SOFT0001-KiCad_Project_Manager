...
class ExplorerTab(QWidget):
    ...
    def setup_ui(self):
        ...
        proj_layout.addWidget(self.lbl_projects)
        proj_layout.addWidget(self.projects_list, 1)
        ...
        info_tabs.setTabPosition(QTabWidget.North)
        info_tabs.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        info_tabs.setStyleSheet("QTabBar::tab { padding: 6px 12px; font-weight: 600; }")

        proj_tab = QWidget()
        proj_tab_layout = QVBoxLayout(proj_tab)
        proj_tab_layout.setContentsMargins(0, 0, 0, 0)
        proj_tab_layout.setSpacing(0)
        proj_tab_layout.addWidget(proj_widget)

        heatmap_tab = QWidget()
        heatmap_tab_layout = QVBoxLayout(heatmap_tab)
        heatmap_tab_layout.setContentsMargins(0, 0, 0, 0)
        heatmap_tab_layout.setSpacing(0)
        heatmap_tab_layout.addWidget(heatmap_group)

        info_tabs.addTab(proj_tab, "Used in Projects")
        info_tabs.addTab(heatmap_tab, "Usage Heatmap")
        selection_layout.addWidget(info_tabs)
        selection_panel.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)
        selection_panel.setMaximumHeight(280)
        ...
        btn_fp_zin = self._make_preview_icon_button(Icons.get_icon(Icons.ZOOM_IN, icon_color), "Zoom In", "fpIconButton", size=28)
        btn_fp_zout = self._make_preview_icon_button(Icons.get_icon(Icons.ZOOM_OUT, icon_color), "Zoom Out", "fpIconButton", size=28)
        btn_fp_zout.clicked.connect(lambda: self.fp_view.zoom(0.8))
        btn_fp_rst = self._make_preview_icon_button(Icons.get_icon(Icons.RESET, icon_color), "Reset View", "fpIconButton", size=34)
        ...
        btn_sym_zin = self._make_preview_icon_button(Icons.get_icon(Icons.ZOOM_IN, icon_color), "Zoom In", "fpIconButton", size=28)
        btn_sym_zout = self._make_preview_icon_button(Icons.get_icon(Icons.ZOOM_OUT, icon_color), "Zoom Out", "fpIconButton", size=28)
        btn_sym_rst = self._make_preview_icon_button(Icons.get_icon(Icons.RESET, icon_color), "Reset View", "fpIconButton", size=34)
        sym_ctrl.addStretch()
    ...
    def _make_preview_icon_button(self, icon, tooltip, name, size=32):
        btn = QPushButton()
